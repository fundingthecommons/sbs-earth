              ...
              <ul style={navStyles} className={`${nav?.navTypeStyle} ${nav?.navAlignment} mt-6 mb-0 flex-grow list-none md:hidden`}>
                ...
                {navItems.map(function (item, index) {
                  ...
                  if (subNavItems) {
                    ...
                    return (
                      <li className="inline-block ml-6" key={index}>
                        <a style={linkStyles} className={"block no-underline"} href={item.link} target={linkTarget(item.link)}>
                          <span className="inline-block w-3 h-3 mr-1 bg-accent2 rounded-full" /> {item.label}
                        </a>
                      </li>
                    )
                  }
